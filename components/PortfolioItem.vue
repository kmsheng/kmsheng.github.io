...
<template>
  <div class="md:flex">
    ...
    <div class="md:pl-5 pt-2 md:pt-0">
      <h3 class="font-bold text-2xl">{{ $t(item.title) }}</h3>
      <div class="whitespace-pre-wrap mt-2 mb-3">{{ $t(item.desc) }}</div>
      ...
      <a v-if="item.url" class="mt-1 inline-block" :href="item.url" target="_blank">{{ $t('view-site') }}</a>
    </div>
  </div>
</template>
